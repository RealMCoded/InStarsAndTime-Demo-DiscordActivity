...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneMenu');
    ...
    Moghunter.scMenu_LVFontSize = Number(Moghunter.parameters['LV FontSize'] || 20);
...
ImageManager.loadMenusFaces2 = function(filename) {
    return this.loadBitmap('img/menus/faces/sprites/', filename, 0, true);
};			
...
ImageManager.loadMenusFaces3 = function(filename) {
    if ($gameVariables.value(2) < 3) {
        return this.loadBitmap('img/menus/faces/menu_act1/', filename, 0, true);
    }
    else if ($gameVariables.value(2) == 3) {
        return this.loadBitmap('img/menus/faces/menu_act3/', filename, 0, true);
    }
        else if ($gameVariables.value(2) == 4) {
        return this.loadBitmap('img/menus/faces/menu_act3/', filename, 0, true);
    }
    else if ($gameVariables.value(2) == 5) {
        return this.loadBitmap('img/menus/faces/menu_act5/', filename, 0, true);
    }
    else if ($gameVariables.value(2) == 6) {
        return this.loadBitmap('img/menus/faces/menu_act6/', filename, 0, true);
    }
...
if ("English" === "Japanese") {
        this._layout = new Sprite(ImageManager.loadMenusMain("Layout_withBonnie_Japanese"));
     }
     else {
        this._layout = new Sprite(ImageManager.loadMenusMain("Layout_withBonnie"));
     }     
...
Scene_Menu.prototype.createCommands = function() {
	 ...
	 for (var i = 0; i < this._comList.length; i++) {
         ...
         //CHANGE this is where to add more space between icons. was at 48 before
		 this._compos[i] = [Moghunter.scMenu_ComX + (65 * i),Moghunter.scMenu_ComY + (65 * h)];
		 this._comzoom[i] = 0;
	...
	// this.updateComField();
     for (var i = 0; i < this._commands.length; i++) {
		  if (this.isComEnabled(i)) {
			   ...
			   if (this._commandWindow.isCurrentItemEnabled()) {this._commands[i].opacity += 100};
			   ...
			   var ny = this._compos[i][1];
  			   if (this._commands[i].opacity > 80 || this._statusWindow.active) {
				   ...
				   if (this._commands[i].opacity < 80 && !this._statusWindow.active) {this._commands[i].opacity = 80}  
			    };
			   if (!this._statusWindow.active && this._commands[i].opacity < 80) {
				   this._commands[i].opacity += 10;
				   if (this._commands[i].opacity > 80) {this._commands[i].opacity = 80}  
			   };
	...
	for (var i = 0; i < this._selection.length; i++) {
		 ...
		 if (i === this._statusWindow._index) {
			 ...
			 if (this._selzoom[i] === 0) {
				 //0this._selection[i].scale.x += 0.015;
...
MCharStatus.prototype.createLVNumber = function() {
	...
	for (var i = 0; i < 3; i++) {
         ...
         this._lv_number[i].fontSize = Moghunter.scMenu_LVFontSize;		 
	};	
